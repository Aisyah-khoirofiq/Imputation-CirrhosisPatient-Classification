...
# --- SMOTE Integration ---
# Use the pipeline from imblearn to include the SMOTE step
...
def train_and_evaluate(_df, target_column, model_name):
    ...
    with st.spinner(f'Applying SMOTE and training {model_name}...'):
...
if st.session_state.df is None:
    ...
    # --- NEW: SMOTE Visualization Section ---
    ...
    with col2:
        ...
        # For visualization, we apply SMOTE to the preprocessed training set
    ...
    # --- END of SMOTE Visualization Section ---
    ...
    with st.form("prediction_form"):
        ...
        input_data = {}
        feature_columns = df_for_training.drop(target_column, axis=1).columns
        ...
        for idx, col in enumerate(feature_columns):
            ...
            with target_col:
                if pd.api.types.is_numeric_dtype(df_for_training[col]):
                    mean_val = df_for_training[col].mean()
                    if pd.isna(mean_val): mean_val = 0.0
                    input_data[col] = st.number_input(f"{col}", value=float(mean_val), key=f"pred_{col}")
                else:
                    options = df_for_training[col].dropna().unique().tolist()
                    if options:
